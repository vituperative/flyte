...
if (!$row || ($row["banned"] == "yes" && !$admin))
	...
	if (!isset($_GET["page"])) {
		...
		echo '<center>';
		echo '<table class="table1" align=center cellpadding="0" cellspacing="0" border="1" width="60%">';
		...
		echo '<tr><td align=center colspan="2" class="td1"><span class="text1"'.$s.'</span></td></tr>';
		...
		tr("Downloads as", $row["save_as"], 0, $rowcount++);
		...
		if (isset($row["cat_name"]))
			tr("Type", $row["cat_name"], 0, $rowcount++);
		else
			tr("Type", "(none selected)", 0, $rowcount++);

		tr("Last seeder", "Last activity " . mkprettytime($row["lastseed"]) . " ago", 0, $rowcount++);
		tr("Size",mksize($row["size"]) . " (" . $row["size"] . " Bytes)", 0, $rowcount++);
		tr("Added (UTC)", $row["added"], 0, $rowcount++);
		tr("Views", $row["views"], 0, $rowcount++);
		tr("Hits", $row["hits"], 0, $rowcount++);
		tr("Downloaded", $row["times_completed"] . " time(s)", 0, $rowcount++);

		$keepget = "";
		$uprow = isset($row["username"]) ? htmlspecialchars($row["username"]) : "<i>unknown</i>";
		if ($owned)
			$uprow .= " $spacer<$editlink>[Edit this torrent]</a>";
		tr("Upped by", $uprow, 1, $rowcount++);
		...
		if (!@$_GET["dllist"]) {
			tr("Peers<br /><a href=\"details.php?id=$id&amp;dllist=1$keepget#seeds\" class=\"sublink\">[See full list]</a>", $row["seeders"] . " seed(s), " . $row["leechers"] . " leeche(s) = " . ($row["seeders"] + $row["leechers"]) . " peer(s) total", 1, $rowcount++);
		}
		else {
			$downloaders = array();
			$seeders = array();
	...
	if (!$count) {
		print("<p class=\"important\" align=\"center\">No comments yet</p>\n");
	}
	else {
